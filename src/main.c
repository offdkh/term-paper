...
int main() {
  ...
  graph_set(g);
  printf("\nДобро пожаловать в программу PF (Path finder).\n");
  while (first) {
    second = 1;
    printf(
    ...
    while (check_for_exist(arrival, g->name) == -1 ||
      ...
      if (check_for_exist(arrival, g->name) == -1 &&
          check_for_exist(departure, g->name) > -1) {
        printf(
            "Город отправки %s отсутствует в списке городов "
            "программы.\n",
            arrival);
        for (int i = 0; i < 20; i++) {
          arrival[i] = '\0';
        }
      } else if (check_for_exist(arrival, g->name) > -1 &&
                 check_for_exist(departure, g->name) == -1) {
        printf(
            "Город прибытия %s отсутствует в списке городов "
            "программы.\n",
            departure);
        for (int i = 0; i < 20; i++) {
          departure[i] = '\0';
