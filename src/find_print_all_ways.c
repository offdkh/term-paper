...
void find_num_of_ways(struct graph* g, struct Tree* tree, int parents[],
                      int start, int end, int (*ways)[V], int* way) {
  if (tree->name != end) {
    for (int i = 0, j = 0; i < V; i++) {
      ...
      if (g->mat[start][i] && chek(parents, name_id) > -1) {
        ...
        tree->child[j]->stat = 1;
        find_num_of_ways(g, tree->child[j], tree->parents, name_id, end, ways,
                         way);
        j++;
      }
    }
...
void command_for_search(struct graph* g, int start, int end, int (*ways)[V],
                        int* way) {
