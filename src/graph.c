...
g->mat[(i - 1) * g->nversh + j - 1] = weight; //// i - индекс откуда, j - индекс
куда, weight - длина пути.
*/
...
void graph_set_edge(struct graph* g, int i, int j,
                    unsigned int weight)  // Назначение веса ребру графа
{
